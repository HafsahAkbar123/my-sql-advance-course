...
SELECT
    t.N AS DayOfYear
    ,DATEADD(DAY, t.N, '2024-12-31') AS TheDate
FROM
    Tally t WHERE N <=366
order by 1
...
SELECT @NumberOfDays as NumerberOfDays;
Select
t.N as DayofYear
    ,DATEADD(DAY, N-1, @StartDate) AS TheDate
...
FROM
    Tally t 
    Left Join #PatientAdmission P ON DATEADD(DAY, t.N, '2023-12-31') = P.AdmittedDate
WHERE
    N <= 8
ORDER BY 1
